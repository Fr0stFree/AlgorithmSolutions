-- Напишите SQL-запрос, который вернет список подходящих квартир. Клиента интересуют все варианты более 1 комнаты,
-- чем дешевле, тем лучше. В итоговой таблице выведите количество комнат, улицу, дом и цену.
...
FROM
    (SELECT
        *,
        RANK() OVER(PARTITION BY rooms ORDER BY price) AS rating
    FROM flats
