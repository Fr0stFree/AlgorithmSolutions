# В таблице orders хранится список заказов компании. Напишите запрос, который в первом столбце (year) — выведет год, во
# втором (status) — статус заказа, в третьем (orders) — общее количество заказов данного статуса за соответствующий год,
# а в четвертом (percent) — процент в рамках соответствующего года. Данные отсортируйте по году и по статусу в
# алфавитном порядке.
...
SELECT
    ...
    status,
    COUNT(id) AS orders,
    COUNT(id) * 100 / SUM(COUNT(id)) OVER(PARTITION BY YEAR(date)) AS percent
FROM orders
